...
using TSP_Transponder.Models.Navigraph;
...
using TSP_Transponder.Models.Flyr;
...
namespace TSP_Transponder
{
    ...
    public partial class App : Application
    {
        ...
        internal static string DocumentsDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), @"Parallel 42\The Skypark\2");
        ...
        internal static string PersistDirectory = Path.Combine(AppDataDirectory, "Persistence");
        ...
        void App_Startup(object sender, StartupEventArgs e)
        {
            ...
            if (Directory.Exists(PersistDirectory))
            {
                var fCount = Directory.GetFiles(PersistDirectory).Length;
                if (fCount == 0)
                {
                    RegenRoutes = true;
                }
        ...
        {
            string AppdataPersist = PersistDirectory;
        ...
        private static int CheckLastVersion()
        {
            try
            {
                ...
                int VersionReturn = TSP_Transponder.Models.PostProcessing.UpgradeProcess.VersionUpgrade();
        ...
        internal static void CheckChannel()
        {
            ...
            if(IsDev)
            {
                ...
                DocumentsDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), @"Parallel 42\The Skypark DEV\2");
                AppDataDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), @"Parallel 42\The Skypark DEV\2");

#if DEBUG
                PersistDirectory = Path.Combine(AppDataDirectory, "Persistence_Debug");
#else
                PersistDirectory = Path.Combine(AppDataDirectory, "Persistence");
#endif
