        ...
        */
        ...
        public cargo_pickup(Adventure Adv, AdventureTemplate Template, Situation Sit, int ID, Dictionary<string, dynamic> Params, action_base Parent) : base(Adv, Template, Sit, ID, Params, Parent)
        {
            ...
            if (Adv != null)
            {
                ...
                {
                    Cargo = AssetsLibrary.CargoItems.Find(x => x.GUID == (string)Params["Cargo"]);
                }

                if(Cargo == null)
                {
                    Cargo = AssetsLibrary.CargoItems.First();
                }
                ...
                foreach (int ActionID in Params["LoadedActions"])
                {
                    action_base ActionObj = CreateAction(Adventure, Template, Situation, ActionID, this);
                    if (ActionObj != null)
                    {
                        LoadedActions.Add(ActionObj);
                    }
                }

                foreach (int ActionID in Params["ForgotActions"])
                {
                    action_base ActionObj = CreateAction(Adventure, Template, Situation, ActionID, this);
                    if (ActionObj != null)
                    {
                        ForgotActions.Add(ActionObj);
                    }
                }

                foreach (int ActionID in Params["EndActions"])
                {
                    action_base ActionObj = CreateAction(Adventure, Template, Situation, ActionID, this);
                    if (ActionObj != null)
                    {
                        EndActions.Add(ActionObj);
                    }
                }
                #endregion
                    ...
                    }) != null)
                    {
                        if (Utils.GetRandom(5) == 3)
                        {
                            if (LoadedActions.Find(x => x.GetType() == typeof(audio_speech_play) || x.GetType() == typeof(audio_effect_play)) == null)
                            {
                                ...
                                Audio.AudioFramework.GetSpeech("characters", "larry/loaded", true, false, false, null, (member, route, msg) =>
                                {
            ...
            if (ItemUnloadingEnd == null && ItemLoaded && !LastTemporalData.IS_SLEW_ACTIVE)
            {
                ...
                Thread loadWait = new Thread(() =>
                {
                    ...
                    GeoPosition UnloadLocation = new GeoPosition(Utils.MapOffsetPosition(
                        ...
                        (float)(((Connectors.SimConnection.Aircraft.WingspanMeters * 0.5) + 2)),
        ...
        public override void Exit()
        {
            ...
            if(!ItemLoaded && !Reminded && !ItemDelivered)
            {
                if (ForgotActions.Find(x => x.GetType() == typeof(audio_speech_play) || x.GetType() == typeof(audio_effect_play)) == null)
                {
                    ...
                    {
                        Audio.AudioFramework.GetSpeech("characters", "brigit/load", true, false, false, null, (member, route, msg) =>
                        {
                            Adventure.Memos.EnsureMember("brigit", "Brigit");
                            Adventure.Memos.Add(new ChatThread.Message()
                            {
                                Type = ChatThread.Message.MessageType.Audio,
                                Param = "characters:" + route,
                    ...
                    {
                        Audio.AudioFramework.GetSpeech("characters", "pablo/load_forgot", true, false, false, null, (member, route, msg) =>
                        {
                            Adventure.Memos.EnsureMember("pablo", "Pablo");
                            Adventure.Memos.Add(new ChatThread.Message()
                            {
                                Type = ChatThread.Message.MessageType.Audio,
                                Param = "characters:" + route,
        ...
        public override Dictionary<string, dynamic> ComputeForTempate(Route route, Dictionary<string, dynamic> Params)
        {
            ...
            if (AssetsLibrary.CargoItemsByTag.ContainsKey(Params["Model"]))
            {
                PossibleItems = AssetsLibrary.CargoItemsByTag[Params["Model"]];
            }
            else
            {
                PossibleItems = AssetsLibrary.CargoItemsByTag["any"];
            }
            List<Cargo> VotingItems = new List<Cargo>();
            ...
            // Create probability
            foreach(Cargo Item in PossibleItems)
            {
            ...
            {
                // Pick a winner
                Cargo WinningCargo = VotingItems[rando.Next(VotingItems.Count - 1)];

                // Define Cargo
                Params["Cargo"] = WinningCargo.GUID;
                Params["Model"] = WinningCargo.Models[rando.Next(WinningCargo.Models.Count)];
                Params["Transport"] = WinningCargo.TransportModels[rando.Next(WinningCargo.TransportModels.Count - 1)];
            }
            return Params;
        }
    }
