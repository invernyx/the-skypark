...
namespace TSP_Transponder.Models.Adventures.Actions
{
    ...
    class trigger_gs_end : action_base
    {
        public List<action_base> SuccessActions = new List<action_base>();
        public List<action_base> FailActions = new List<action_base>();

        public int LinkID = -1;
        public trigger_gs_start Link = null;
        public float CancelRange = 0;

        public trigger_gs_end(Adventure Adv, AdventureTemplate Template, Situation Sit, int ID, Dictionary<string, dynamic> Params, action_base Parent) : base(Adv, Template, Sit, ID, Params, Parent)
        {
            ActionName = "trigger_gs_end";
            ...
            if (Adv != null)
            {
                ...
                {
                    if (action.Value.GetType() == typeof(trigger_gs_start))
                    {
                        trigger_gs_start TestAction = (trigger_gs_start)action.Value;
