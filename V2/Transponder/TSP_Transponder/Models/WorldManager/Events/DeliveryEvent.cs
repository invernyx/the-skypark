                            ...
                            if(LastTemporalData.SURFACE_TYPE != Surface.Water)
                            {
                                if (NodesClusters.Count > 0)
                                {
                                    ...
                                    if (SortedParkings.Count > 0)
                                    {
                                        ...
                                        List<Node> Exclusion = SortedCluster.Keys.Where(x => Utils.MapCalcDist(x.Location, AircraftLocation) > Connectors.SimConnection.Aircraft.WingspanMeters || x.Type == NodeType.Parking).ToList();
                                        ...
                                        KeyValuePair<List<Node>, GeoLoc>? ClosestSegment = PathFindingHelpers.GetClosestSegment(SortedCluster.Keys.ToList(), AircraftLocation, Connectors.SimConnection.Aircraft.WingspanMeters);
                                        ...
                                        if (ClosestSegment != null && DistanceToClosestSegment < Utils.MapCalcDist(SortedCluster.Keys.FirstOrDefault().Location, AircraftLocation, Utils.DistanceUnit.Meters) && DistanceToClosestSegment > Connectors.SimConnection.Aircraft.WingspanMeters)
                                        {
                                            Node nn = new Node()
                                            {
                                        ...
                                        {
                                            SearchMap.EndNode = SortedCluster.Keys.Where(x => Utils.MapCalcDist(x.Location, Connectors.SimConnection.LastTemporalData.PLANE_LOCATION, Utils.DistanceUnit.Meters) > Connectors.SimConnection.Aircraft.WingspanMeters).FirstOrDefault();
                                        }
                                        #endregion
                                            ...
                                            case double n0 when (n0 > 0 && n0 <= 90): // Front Right
                                                {
                                                    VEH1_Move.AddWP(new VehMovement.Waypoint()
                                                    {
                                                        Location = Utils.MapOffsetPosition(
                                                            ...
                                                            (float)((Connectors.SimConnection.Aircraft.WingspanMeters * 2)),
                                            ...
                                            case double n1 when (n1 > 130 && n1 <= 180): // Back Right
                                                {
                                                    VEH1_Move.AddWP(new VehMovement.Waypoint()
                                                    {
                                                        Location = Utils.MapOffsetPosition(
                                                            Connectors.SimConnection.LastTemporalData.PLANE_LOCATION,
                                                            (float)((Connectors.SimConnection.Aircraft.WingspanMeters * 1.1)),
