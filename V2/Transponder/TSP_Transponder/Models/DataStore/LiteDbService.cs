    ...
    class LiteDbService
    {
        ...
        internal static void Startup()
        {
            BsonMapper.Global.ResolveMember = (type, memberInfo, memberMapper) =>
            {
                ...
                {
                    memberMapper.Serialize = (obj, mapper) => new BsonValue(App.GetDescription((Enum)obj));
                    memberMapper.Deserialize = (value, mapper) => App.GetEnum(memberMapper.UnderlyingType, value);
                }
            };
            ...
            #region Init Airports
           // if (DBApt == null) { DBApt = new LiteDbService(Path.Combine(UserData.Get("install"), "Transponder", "Airports.dat"), null, "6ed9c54a937e", false); }
            #endregion

            #region Adventures Library
            if (!Directory.Exists(Path.Combine(App.AppDataDirectory, "Persistence")))
            {
