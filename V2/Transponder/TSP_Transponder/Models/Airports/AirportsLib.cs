...
namespace TSP_Transponder.Models.Airports
{
    public partial class AirportsLib
    {
        ...
        public void Startup()
        {
            ...
            //APTScan.AssignRadius(AllAirports);
            ...
            foreach (string Geo in matrix)
            {
                ...
                foreach (Airport Airport in GeoAirports)
                {
                    if (!ReturnedTiles[Sim.NameStandard][Geo].ContainsKey(Airport.ICAO))
                    {
                        if (IsDetailed)
                        {
                            ReturnedTiles[Sim.NameStandard][Geo].Add(Airport.ICAO, Airport.ToDetailed());
                        }
                        else
                        {
                            ReturnedTiles[Sim.NameStandard][Geo].Add(Airport.ICAO, Airport.ToSummary(IsDetailed));
                        }
                        QueueCount++;
                    }
                }
        ...
        public void Command(SocketClient Socket, string[] StructSplit, Dictionary<string, dynamic> structure)
        {
            ...
            switch (StructSplit[1])
            {
                case "from-icaos":
                    {
                        Dictionary<string, Dictionary<string, dynamic>> ReturnedTiles = new Dictionary<string, Dictionary<string, dynamic>>();
                        ...
                        Socket.SendMessage("airports:from-icaos", App.JSSerializer.Serialize(ReturnedTiles), (Dictionary<string, dynamic>)structure["meta"]);
                        break;
                    }
                ...
                case "from-coords":
                    {
                        ...
                        bool Detailed = (bool)payload_struct["detailed"];
                        double Radius = (double)payload_struct["detail"] * 0.06;
                        ...
                        while(LonAt < LonMax)
                        {
                            ...
                            while (LatAt < LatMax)
                            {
                                ...
                                foreach (var airport in GetAirportsByTile(geo))
                                {
                                    ...
                                    {
                                        outlist.Add(airport.ToSummary(Detailed));
                                        Count++;
                                    }
                                }
                        ...
                        Socket.SendMessage("airports:from-coords", App.JSSerializer.Serialize(ReturnedTiles), (Dictionary<string, dynamic>)structure["meta"]);
                        break;
                    }
            }
        }
        ...
        public List<KeyValuePair<double, Airport>> GetAirportByRange(GeoLoc location, float rangeKM)
        {
            ...
            {
                double aptDist = Utils.MapCalcDist(airport.Location, location);
        ...
        public Dictionary<string, Dictionary<string, dynamic>> ToDetailed(List<Airport> list)
        {
            Dictionary<string, Dictionary<string, dynamic>> rt = new Dictionary<string, Dictionary<string, dynamic>>();
            foreach (Airport apt in list)
            {
                if (rt.ContainsKey(apt.ICAO))
                {
                    continue;
                }

                rt.Add(apt.ICAO, apt.ToDetailed());
            }
