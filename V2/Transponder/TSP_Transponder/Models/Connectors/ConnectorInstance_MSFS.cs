...
#pragma warning restore IDE1006
        ...
        private bool PayloadStationsInitialized = false;
        ...
        public override void Disconnect()
        {
            ...
            if (SimConnectModule != null)
            {
                ...
                PayloadStationsInitialized = false;
                IsRunning = false;
                IsPaused = false;
                TryingConnect = false;
                AircraftDirectory = "";
        ...
        public override void SendMessage(string msg, float duration)
        {
            lock(MessageQueue)
            {
            ...
            Task.Factory.StartNew(() => {
                ...
                while (MessageQueue.Count > 0)
                {
                    if(SimConnectModule != null)
                    {
                        ...
                        SimConnectModule.Text(SIMCONNECT_TEXT_TYPE.PRINT_WHITE, MessageQueue[0].Key, EVENT_ID.ShowMessage, MessageQueue[0].Value);
        ...
        public override void ReadAircraftConfig(Simulator Simulator, string DirectoryName, string DirectoryFull, Action<string, string, string> Callback)
        {
            foreach(string Dir in Simulator.ConfigsFolders)
            {
                try
                {
                    ...
                    {
                        string p = GetInstallDir(optFilePath);

                        List<string> Manifests = Directory.GetFiles(p, "aircraft.cfg", SearchOption.AllDirectories).ToList();
                        ...
                        if (AircraftCFGs.Count > 0)
                        {
                            foreach (string AircraftCFG in AircraftCFGs)
                            {
                                ...
                                if(File.Exists(manifestPath))
                                {
                                    ...
                                    Callback(Manufacturer, Creator, Model);
                                    return;
                                }
                            }
                        }
        ...
        public override void SendPayload(int Station, float WeightPounds)
        {
            PayloadStruct existingPayload = new PayloadStruct()
            {
                STATION = WeightPounds,
            };
        ...
        private void SetupEventHandlers()
        {
            try
            {
                ...
                SimConnectModule.AddToDataDefinition(DEFINITIONS.LoopFrameStruct, "VELOCITY WORLD X", "FEET PER SECOND", SIMCONNECT_DATATYPE.FLOAT64, 0f, SimConnect.SIMCONNECT_UNUSED);
                SimConnectModule.AddToDataDefinition(DEFINITIONS.LoopFrameStruct, "VELOCITY WORLD Y", "FEET PER SECOND", SIMCONNECT_DATATYPE.FLOAT64, 0f, SimConnect.SIMCONNECT_UNUSED);
                SimConnectModule.AddToDataDefinition(DEFINITIONS.LoopFrameStruct, "VELOCITY WORLD Z", "FEET PER SECOND", SIMCONNECT_DATATYPE.FLOAT64, 0f, SimConnect.SIMCONNECT_UNUSED);
                ...
                SimConnectModule.AddToDataDefinition(DEFINITIONS.LoopFrameStruct, "AMBIENT IN CLOUD", "BOOL", SIMCONNECT_DATATYPE.FLOAT64, 0f, SimConnect.SIMCONNECT_UNUSED);
                ...
                case DATA_REQUESTS.Sim_Frame_Data:
                    {
                        ...
                        if (Aircraft != null)
                        {
                            ...
                            if(SimData.LoopFrame.ABS_TIME != LastTemporalData.ABS_TIME)
                            {
                                ...
                                LastTemporalData.VELOCITY_WORLD_X = SimData.LoopFrame.VELOCITY_WORLD_X;
                                LastTemporalData.VELOCITY_WORLD_Y = SimData.LoopFrame.VELOCITY_WORLD_Y;
                                LastTemporalData.VELOCITY_WORLD_Z = SimData.LoopFrame.VELOCITY_WORLD_Z;
                            ...
                            IsLoaded = ((Math.Round(LastTemporalData.PLANE_LOCATION.Lat, 3) != 0 && Math.Round(LastTemporalData.PLANE_LOCATION.Lon, 3) != 0) || Math.Round(LastTemporalData.SURFACE_RELATIVE_GROUND_SPEED) > 70);

                            if(IsLoaded && !PayloadStationsInitialized)
                            {
                ...
                case DATA_REQUESTS.Aircraft_Init:
                    {
                        ...
                        Aircraft = FleetService.CreateAircraft(
                            ...
                            SimData.AircraftData.WING_SPAN_METERS > 10 ? SimData.AircraftData.WING_SPAN_METERS : 20, 
        ...
        private void CameraTimerTick(int id, int msg, int user, int param1, int param2)
        {
            if (IsRunning)
            {
                ...
                {
                    if (SimConnection.APIResponseTime.Count >= 30 && SimConnection.APIResponseTime.Average() < 70)// && SimData.FastLoop.SIM_RUNTIME > 0)
                    {
        ...
        private void EnablePayloadStations(AircraftInstance Acf)
        {
            return;
            /*
            PayloadStationsInitialized = true;
            ...
            {
                i++;
                SimConnectModule.AddToDataDefinition((DEFINITIONS)(1000 + i), "PAYLOAD STATION WEIGHT:" + i, "Pounds", SIMCONNECT_DATATYPE.FLOAT64, 0f, SimConnect.SIMCONNECT_UNUSED);
            }
