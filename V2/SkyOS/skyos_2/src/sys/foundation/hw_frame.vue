<template>
	...
	[
	 	!window_device ? 'is-device' : 'is-window',
		{
			...
			'device-apple': this.$root.$data.state.device.isAppleWebkit
		}
	]">
		<div v-if="$root.$data.state.device.isElectron && $os.activeAppRouter">
			<div class="hw_frame">
				<div class="hw_frame_bezel" @dblclick="dClickBezel" @mousedown="onMouseDown" ref="hw_frame_bezel">
				...
				<div class="hw_frame_screen">
					<App class="hw_frame_pixels" :class="this.$root.$data.config.ui.theme"/>
				</div>
			...
			<div class="hw_sizes">
				<div class="hw_resize" :style="'height:' + ($root.$data.config.ui.device_resize.size_2 * 80) + 'px'" @click="clickResize(2)">
					<div></div>
				</div>
				<div class="hw_resize" :style="'height:' + ($root.$data.config.ui.device_resize.size_1 * 80) + 'px'" @click="clickResize(1)">
					<div></div>
				</div>
			</div>
		</div>
		<App class="hw_blank" :class="this.$root.$data.config.ui.theme" v-else/>
	</div>
</template>
...
export default Vue.extend({
	...
	methods: {
		handlePin() {
			this.ipcR.send('a-msg', 'pin');
		},

		changeSize(change :any) {
			this.ipcR.send('a-msg', 'zoom', change);
		},

		handleMin() {
			this.ipcR.send('a-msg', 'min');
		},

		handleCls() {
			this.ready = false;
			this.ipcR.send('a-msg', 'cls');
		},
		...
		releaseP42() {
			if(this.p42Held != null) {
				clearTimeout(this.p42Held);
				this.p42Held = null;
				window.open("https://tfdidesign.com/");
			}
		},
		...
		dClickBezel(e) {
			...
			if(side) {
				this.ipcR.send('a-msg', 'stow', side);
			}
		},
		...
		moveWindow(e) {
			if(this.moved) {
				this.ipcR.send('a-msg', 'moving', [this.mouseX, this.mouseY]);
			}
			this.animationId = requestAnimationFrame(this.moveWindow);
		},

		clickResize(sel :number) {
			this.$root.$data.config.ui.device_resize.selected = sel;
			switch(sel) {
				case 1: {
					this.changeSize(this.$root.$data.config.ui.device_resize.size_1);
					break;
				}
				case 2: {
					this.changeSize(this.$root.$data.config.ui.device_resize.size_2);
					break;
				}
			}
		},
	...
	created() {
		if (navigator.userAgent.indexOf('Electron') >= 0) {
			...
			this.$root.$data.state.device.isElectron = true;
			...
			ipcRenderer.on('asynchronous-reply', (event, arg, arg2) => {
				...
				switch(argSpl[0]){
					...
					case 'framed': {
						this.$os.setConfig(['ui','framed'], arg2);
						break;
					}
					case 'zoom': {
						switch(this.$root.$data.config.ui.device_resize.selected) {
							case 1: {
								this.$os.setConfig(['ui','device_resize','size_1'], Eljs.round(arg2, 2));
								break;
							}
							case 2: {
								this.$os.setConfig(['ui','device_resize','size_2'], Eljs.round(arg2, 2));
								break;
							}
						}
						break;
					...
					case 'pin': {
						...
						this.$root.$data.config.ui.topmost = state;
						(this.$root as any).stateSave();
						break;
					}
				}
			...
			setTimeout(() => {
				(this.$root as any).setTopmost(this.$root.$data.config.ui.topmost);
			}, 1000);
		}

		this.$root.$on('statechange', (path :string[], value :any) => {
			switch(path[0]) {
				case 'device': {
					switch(path[1]) {
						case 'windowed': {
							this.window_device = value;
							break;
						}
						case 'frameless': {
							this.window_frameless = value;
							break;
						}
					}
					break;
...
.is-device {
	...
	.hw_frame {
		...
		&_bezel {
			...
			width: 586px;
			height: 770px;
			//background-image: url(../assets/hardware/bezel.svg);
			//pointer-events: none;
			path {
		...
		&_screen {
			...
			-webkit-app-region: no-drag;
			-webkit-mask: url(../assets/hardware/screen.svg);
		...
		&_outline {
			...
			width: 586px;
			height: 770px;
			background-image: url(../assets/hardware/chrome.svg);
			background-repeat: no-repeat;
			pointer-events: none;
		}
		&_pixels {
	...
	.w_controls {
		...
		.w_control {
			...
			&_p42 {
				...
				background-image: url(../assets/hardware/tfdi_design.png);
				width: 36px;
