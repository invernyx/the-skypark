		...
		<content_controls_stack :translucent="true" :scroller_offset="{ top: 10 }">
			<template v-slot:content>
				<div class="helper_edge_padding_half">
					<div class="background-map" v-if="md.data.Airport">
						<div class="background-map-image" :style="'background-image:url(' + $os.getCDN('images', 'airports/' + md.data.Airport.ICAO + '.jpg') + ')'"></div>
			...
			<template v-slot:tab>
				<div class="helper_edge_margin_half">
					<button_action @click.native="close" class="cancel">Close</button_action>
				</div>
			</template>
...
export default Vue.extend({
	...
	methods: {
		...
		sendReport(type :string) {
			this.$os.TrackEvent("ReportAirport", this.md.data.Airport.ICAO, type);
