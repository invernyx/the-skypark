...
namespace TSP_Transponder.Models.WorldManager
{
    internal class World
    {
        ...
        internal static void Connect()
        {
            lock (OBJs)
            {
                ...
                {
                    if(OBJ.CullingVisible && OBJ.IsEnabled)
                    {
                        OBJ.IsEnabled = true;
                    }
                }
        ...
        {
            CancelInterceptOBJs.Clear();

            lock (UnconfirmedOBJs)
            {
                foreach (var OBJ in UnconfirmedOBJs)
                {
                    lock(OBJs)
                    {
                        OBJs.Add(OBJ);
            ...
            lock (OBJs)
            {
                foreach (var OBJ in new List<Scene_Obj>(OBJs))
                {
                    OBJ.IsEnabled = false;
                }
                OBJs.Clear();
        ...
        internal static void ConfirmID(long uid, uint? SimID)
        {
            ...
            lock (UnconfirmedOBJs)
            {
                ...
                {
                    UnconfirmedOBJ.ConfirmID((uint)SimID);
            ...
            lock (OBJs)
            {
                lock (CancelInterceptOBJs)
                {
                    PendingDeletionOBJ = CancelInterceptOBJs.Find(x => x.UID == uid);
                    if (PendingDeletionOBJ != null)
                    {
                        Console.WriteLine("//////// Removing Pending object ID " + uid + " from Layer " + PendingDeletionOBJ.Layer);
                        lock (CancelInterceptOBJs)
                        {
                            CancelInterceptOBJs.Remove(PendingDeletionOBJ);
                        }
                        PendingDeletionOBJ.IsEnabled = false;
                    }
                }
            }
            ...
            lock (UnconfirmedFXs)
            {
                ...
                lock (CancelInterceptFXs)
                {
                    Scene_Fx PendingDeletionFX = CancelInterceptFXs.Find(x => x.UID == uid);
            ...
            lock (UnconfirmedAFXs)
            {
                ...
                lock (CancelInterceptFXs)
                {
                    Attached_Fx PendingDeletionAFX = CancelInterceptAFXs.Find(x => x.UID == uid);
        ...
        internal static void Dispose()
        {
            ...
            foreach(var obj in OBJs.Concat(UnconfirmedOBJs).Concat(CancelInterceptOBJs))
            {
                obj.Dispose();
            }
    ...
    internal class Attached_Fx
    {
        ...
        internal void Remove()
        {
            ...
            {
                lock (World.CancelInterceptAFXs)
                {
                    World.CancelInterceptAFXs.Add(this);
                }
            }
        }
        ...
        private void CancelSend(bool state)
        {
            if (state)
            {
                lock (World.CancelInterceptAFXs)
                {
                    if (!World.CancelInterceptAFXs.Contains(this))
                    {
                        World.CancelInterceptAFXs.Add(this);
                    }
                }
            ...
            else
            {
                lock (World.CancelInterceptAFXs)
                {
                    if (World.CancelInterceptAFXs.Contains(this))
                    {
                        World.CancelInterceptAFXs.Remove(this);
                    }
                }
            }
        }
    ...
    internal class Scene_Fx
    {
        ...
        internal void Remove()
        {
            ...
            {
                lock (World.CancelInterceptFXs)
                {
                    World.CancelInterceptFXs.Add(this);
                }
            }
        }
        ...
        private void CancelSend(bool state)
        {
            if (state)
            {
                lock (World.CancelInterceptFXs)
                {
                    if (!World.CancelInterceptFXs.Contains(this))
                    {
                        World.CancelInterceptFXs.Add(this);
                    }
                }
            ...
            else
            {
                lock (World.CancelInterceptFXs)
                {
                    if (World.CancelInterceptFXs.Contains(this))
                    {
                        World.CancelInterceptFXs.Remove(this);
                    }
                }
            }
        }
    ...
    public class Scene_Obj
    {
            ...
            set
            {
                if(_CullingVisible != value)
                {
                            ...
                            lock (World.UnconfirmedOBJs)
                            {
                                ...
                                else
                                {
                                    if (ConnectedInstance != null)
                                    {
                                        ...
                                        ConnectedInstance.CreateSimObject(UID, File, Location, ObjType);
                                    }
                                }
                    ...
                    else
                    {
                        ...
                        {
                            Console.WriteLine("++++++++ Unable to destroy Simobject: " + ex.Message);
                        }
                    }
                }
            }
        ...
        internal float CullingDistMin = 0;
        internal float CullingDistMax = 0.5f;
        ...
        internal void ConfirmID(uint? id)
        {                      
            Console.WriteLine("++++++++ Confirmed Simobject " + File + " with ID " + id + " from Layer " + Layer);
            if (id != null)
            {
                SimID = (uint)id;
        ...
        {
            double Dist = Utils.MapCalcDist(new GeoLoc(Location), LastTemporalData.PLANE_LOCATION, Utils.DistanceUnit.Kilometers);
            if (Dist < CullingDistMax && Dist > CullingDistMin)
            {
                return true;
            }
            return false;
        ...
        private void CancelSend(bool state)
        {
            if(state)
            {
                lock (World.CancelInterceptOBJs)
                {
                    if (!World.CancelInterceptOBJs.Contains(this))
                    {
                        World.CancelInterceptOBJs.Add(this);
                    }
                }
            }
            else
            {
                lock (World.CancelInterceptOBJs)
                {
                    if (World.CancelInterceptOBJs.Contains(this))
                    {
                        World.CancelInterceptOBJs.Remove(this);
                    }
                }
            }
        }
