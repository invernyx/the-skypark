...
namespace TSP_Transponder
{
    public static class Utils
    {
        ...
        public static float MapCalcDistFloat(float lat1, float lon1, float lat2, float lon2, DistanceUnit unit = DistanceUnit.Kilometers)
        {
            float dist = 0;
            float rlat1 = 0;
            float rlat2 = 0;
            ...
            dist = (float)(Math.Sin(rlat1) * Math.Sin(rlat2) + Math.Cos(rlat1) * Math.Cos(rlat2) * Math.Cos(rtheta));
        ...
        public static double MapCalcDist(GeoLoc Pos1, GeoLoc Pos2, DistanceUnit unit = DistanceUnit.Kilometers, bool UseFloat = false)
        {
            if(Pos2 != null && Pos1 != null)
            {
                if(UseFloat)
                {
                    return MapCalcDistFloat((float)Pos1.Lat, (float)Pos1.Lon, (float)Pos2.Lat, (float)Pos2.Lon, unit);
                }
                else
                {
                    return MapCalcDist(Pos1.Lat, Pos1.Lon, Pos2.Lat, Pos2.Lon, unit);
                }
            }
            else
            {
        ...
        public static float DistPointToPoly(List<GeoLoc> vs, GeoLoc point)
        {
            ...
            float x = (float)point.Lon;
            float y = (float)point.Lat;

            bool inside = false;
            float closest = float.MaxValue;
            for (int i = 0, j = vs.Count - 1; i < vs.Count; j = i++)
            {
                float xi = (float)vs[i].Lon;
                float yi = (float)vs[i].Lat;
                float xj = (float)vs[j].Lon;
                float yj = (float)vs[j].Lat;

                if (xi != xj || yi != yj)
                {
                    Vector2 Closest = GetClosestPointOnLineSegment(new Vector2(xi, yi), new Vector2(xj, yj), new Vector2(x, y));
                    float dist = MapCalcDistFloat(Closest.Y, Closest.X, y, x, DistanceUnit.Kilometers);
                    if (dist < closest)
                    {
                        closest = dist;
                    }
    ...
    public class WeightedRandom<T>
    {
        ...
        {
            entries.Remove(entries.Find(x => x.item.Equals(entry)));
            accumulatedWeight = 0;
            foreach (var e in entries)
            {
                accumulatedWeight += e.weight;
