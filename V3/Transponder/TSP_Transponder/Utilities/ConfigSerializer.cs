...
namespace TSP_Transponder.Utilities
{
    class ConfigSerializer<T>
    {
        ...
        public ConfigSerializer(T source) {
            Source = source;
        }

        public ConfigSerializer(T source, Dictionary<string, dynamic> state)
        {
            Source = source;
            State = state;
        }
        ...
        public void Set(string entry, Func<dynamic, dynamic> modifier = null)
        {
            ...
            if (State.ContainsKey(entry))
            {
                ...
                {
                    return (x.GetCustomAttributes(typeof(ConfigSerializerField), false) as ConfigSerializerField[]).Where(x1 => x1.Name == entry).Count() > 0;
                }))
                {
    ...
    public class ConfigSerializerField : System.Attribute
    {
        public string Name;
        public bool IsKey;
        public ConfigSerializerField(string name, bool isKey = false)
        {
            Name = name;
            IsKey = isKey;
        }
