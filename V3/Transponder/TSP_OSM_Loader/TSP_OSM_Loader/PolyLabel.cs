...
namespace TSP_OSM_Loader
{
    class PolyLabel
    {
        ...
        public static float[][] ConvertPolygonToFloatArray(List<List<double>> poly)
        {
            // this is a float[][]
            var polygon = new float[poly.Count][];
            var pointCount = 0;
            foreach (var point in poly)
            {
                polygon[pointCount] = new float[2];
                polygon[pointCount][1] = (float)point[1];
                polygon[pointCount][0] = (float)point[0];
                pointCount++;
            }
            return polygon;
        }
