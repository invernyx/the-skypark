<template>
	<div class="contract" :class="{
		'selected': selected ? selected.id == contract.id : false,
		'unavailable': contract.expired,
		'expired': contract.expired,
		'invisible': !visible
		 }" @click="select()" :data-contract="contract.id" :data-scrollvisibility="'hidden'">

		<div class="background" :class="{ 'loading': !color_bright }" v-if="contract.image_url.length" :style="{ 'background-color': background_color }">
		...
		<div class="content">
			<div class="columns header">
				...
				<div class="column column_narrow payloads" v-if="contract.manifests.max_cargo_pods + contract.manifests.max_pax_seats > 0">
					<div class="payload payload-pax" v-if="contract.manifests.max_pax_seats"></div>
					<div class="payload payload-cargo" v-if="contract.manifests.max_cargo_pods"></div>
				</div>
			</div>
			<div class="body">
				<div class="columns">
				...
				<div class="columns" v-if="contract.limits.length">
					<div class="column">
						<div class="restrictions">
							<ContractLimits :contract="contract"/>
						</div>
					</div>
				</div>
				<div class="columns">
					<div class="column column_narrow column_justify_center">
						<div class="aircraft"><RecomAircraft :contract="contract" :small="true" /></div>
					</div>
					<div></div>
					<div class="column column_align_end">
	...
	watch: {
		...
		scroll_visible: {
			handler(newValue, oldValue) {
				...
				clearTimeout(this.colorize_timer);

				if(newValue && !this.load) {
					this.colorize_timer = setTimeout(() => {
						this.load = true;
