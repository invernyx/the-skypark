<template>
	<scroll_view :sid="'p42_yoflight_contract'" :scroller_offset="{ top: 36, bottom: 30 }" @scroll="$emit('scroll', $event)">
		<div class="app-panel-wrap">
			<div class="app-panel-content" ref="app_panel_content" :class="{ 'is-open': open }" :style="{ 'padding-top': 'var(--spacing-top)' }">
				...
				<div class="app-panel-hit" @mouseenter="$emit('can_scroll', true)" @mouseleave="$emit('can_scroll', false)">
					...
					<ContractPanel :app="app" :loading="loading" :contract="selected_contract" :app_panel_content="$refs.app_panel_content" />
...
export default Vue.extend({
	...
	methods: {
		...
		listener_map(wsmsg: any) {
			switch(wsmsg.name){
				case 'interact': {
					this.$os.scrollView.scroll_to('p42_yoflight_contract', 0, 0, 300);
					break;
				}
			}
		},
		listener_navigate(wsmsg :any) {
			switch(wsmsg.name){
				case 'to': {
					if(this.$route.name == 'p42_yoflight_contract') {
						this.init();
					} else {
						this.$os.contract_service.dispose_single(this.selected_contract);
					}
		...
		listener_os(wsmsg :any) {
			switch(wsmsg.name){
				case 'uncover': {
					this.$os.scrollView.scroll_to('p42_yoflight_contract', 0, 0, 100);
					break;
				}
				case 'covered': {
