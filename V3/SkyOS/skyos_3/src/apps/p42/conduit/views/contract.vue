<template>
	<scroll_view :sid="'p42_conduit_contract'" :scroller_offset="{ top: 36, bottom: 30 }" @scroll="$emit('scroll', $event)">
...
export default Vue.extend({
	...
	methods: {
		...
		listener_map(wsmsg: any) {
			switch(wsmsg.name){
				case 'interact': {
					this.$os.scrollView.scroll_to('p42_conduit_contract', 0, 0, 300);
					break;
				}
			}
		},
		listener_navigate(wsmsg :any) {
			switch(wsmsg.name){
				case 'to': {
					if(this.$route.name == 'p42_conduit_contract') {
						this.init();
					} else {
						this.$os.contract_service.dispose_single(this.selected_contract);
					}
		...
		listener_os(wsmsg :any) {
			switch(wsmsg.name){
				case 'uncover': {
					this.$os.scrollView.scroll_to('p42_conduit_contract', 0, 0, 100);
					break;
				}
				case 'covered': {
