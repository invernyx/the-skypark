<template>
	<scroll_view :sid="'p42_conduit_manifest_contract'" :scroller_offset="{ top: 36, bottom: 30 }" @scroll="$emit('scroll', $event)">
		<div class="app-panel-wrap">
			<div class="app-panel-content" ref="app_panel_content" :class="{ 'is-open': open }" :style="{ 'padding-top': 'var(--spacing-top)' }">
				...
				<div class="app-panel-hit" @mouseenter="$emit('can_scroll', true)" @mouseleave="$emit('can_scroll', false)">
					<ManifestPanel v-if="selected_contract" :app="app" :loading="loading" :contract="selected_contract" :manifest="selected_contract.manifests" :manifest_state="selected_contract.manifests_state" :app_panel_content="$refs.app_panel_content" />
				</div>
			</div>
		</div>
	</scroll_view>
...
import ManifestPanel from "@/sys/components/manifest/manifest_panel.vue"
...
export default Vue.extend({
	...
	methods: {
		...
		listener_map(wsmsg: any) {
			switch(wsmsg.name){
				case 'interact': {
					this.$os.scrollView.scroll_to('p42_conduit_manifest_contract', 0, 0, 300);
					break;
				}
			}
		},
		listener_navigate(wsmsg :any) {
			switch(wsmsg.name){
				case 'to': {
					if(this.$route.name == 'p42_conduit_manifest_contract') {
						this.init();
					} else {
						this.$os.contract_service.dispose_single(this.selected_contract);
					}
		...
		listener_os(wsmsg :any) {
			switch(wsmsg.name){
				case 'uncover': {
					this.$os.scrollView.scroll_to('p42_conduit_manifest_contract', 0, 0, 100);
					break;
				}
				case 'covered': {
		...
		listenerWs(wsmsg: any) {
			switch(wsmsg.name[0]){
				case 'adventure': {
					switch(wsmsg.name[1]) {
						case 'manifest': {
							if(this.selected_contract.id == wsmsg.payload.manifests.contract.id){
								this.selected_contract.manifests = wsmsg.payload.manifests;
								this.app.events.emitter.emit('payload_updated');
							}
							break;
						}
						case 'manifest_state': {
							if(this.selected_contract.id == wsmsg.payload.manifests_state.contract.id){
								this.selected_contract.manifests_state = wsmsg.payload.manifests_state;
								this.app.events.emitter.emit('payload_updated');
							}
							break;
						}
