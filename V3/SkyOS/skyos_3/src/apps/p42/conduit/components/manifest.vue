<template>
	<div class="contract" :class="{ 'selected': selected ? selected.id == contract.id : false }" @click="$emit('details')" :data-contract="contract.id" :data-scrollvisibility="'hidden'">
		...
		<div class="content">
			...
			<div class="body">
				<div class="columns" v-if="contract.manifests.total_weight > 0">
					<div class="column">
						<div class="payloads">
							<div class="payload" v-for="(cargo, index) in (contract.manifests.cargo).map(x => x.manifests).flat(1)" v-bind:key="index">
								<div class="columns">
									<div class="column column_narrow column_justify_center">
										<div class="payload_image payload_image_cargo"></div>
									</div>
									<div class="column column_justify_center">
										<div class="payload_description">{{ cargo.total_quantity }} of  {{ cargo.name }}</div>
										<div class="payload_weight"><weight :amount="cargo.total_weight" :decimals="0"/></div>
									</div>
								</div>
							</div>
							<div class="payload" v-for="(pax, index) in (contract.manifests.pax).map(x => x.manifests).flat(1)" v-bind:key="index">
								<div class="columns">
									<div class="column column_narrow column_justify_center">
										<div class="payload_image payload_image_pax"></div>
									</div>
									<div class="column column_justify_center">
										<div class="payload_description">{{ pax.total_quantity }} {{ pax.name }}</div>
										<div class="payload_weight"><weight :amount="pax.total_weight" :decimals="0"/></div>
									</div>
								</div>
							</div>
						</div>
...
.contract {
	...
	.theme--bright & {
		...
		&.selected {
			&::after {
				border-color: $ui_colors_bright_button_gold;
			}
			.background {
				& > div {
					opacity: 0.2;
