		...
		<transition
			...
			>
				<keep-alive :max="3" :include="app_cache_list">
					<router-view class="os-app" :root="root" name="load"/>
				</keep-alive>
		</transition>
	</div>
</template>
...
<script lang="ts">
import Vue from "vue";
import { AppType, AppInfo } from "../foundation/app_model";
import Map from "../components/maps/os_map.vue"
...
export default Vue.extend({
	...
	methods: {
		...
		setTransitionType() {
			...
			switch(this.app_transition_in.type){
				//case AppType.SLEEP: { // IN
				//	switch(this.transition_out.type){
				//		case AppType.HOME: { // Out
				//			this.transition_type_in = Transition.NONE;
				//			this.transition_type_out = Transition.ZOOM;
				//			break;
				//		}
				//		default: { // Out
				//			this.transition_type_in = Transition.NONE;
				//			this.transition_type_out = Transition.FADE;
				//			break;
				//		}
				//	}
				//	break;
				...
				//			this.transition_type_in = Transition.NONE;
				//			this.transition_type_out = Transition.NONE;
				//			break;
				//		}
				//		default: { // Out
				//			this.transition_type_in = Transition.NONE;
				...
				//		case AppType.SLEEP: { // Out
				//			this.transition_type_in = Transition.ZOOM;
				//			this.transition_type_out = Transition.NONE;
				//			break;
				//		}
				//		default: { // Out
				...
				//}
				default: {
					switch(this.app_transition_out.type){
						//case AppType.SLEEP: { // Out
						//	this.transition_type_in = Transition.ZOOM;
						//	this.transition_type_out = Transition.NONE;
						//	break;
						//}
						default: { // Out
							this.app_transition_type_in = Transition.SLIDE;
							this.app_transition_type_out = Transition.SLIDE;
							break;
						}
					}
					break;
		...
		transition_beforeEnter(el: HTMLElement) {
			...
			this.app_transition_in = this.$os.apps.find((x: AppInfo) => x.vendor + "_" + x.ident == el.dataset.appname);
		},
		transition_enter(el: HTMLElement, done: Function) {
			...
			switch(this.app_transition_type_in){
				...
				case Transition.SLIDE: {
					this.transition_in_class_pre = "transition-in-slide-pre";
					this.transition_in_class = "transition-in-slide";
					this.transition_in_class_in = "";
					this.transition_in_duraction = 1000;
					break;
				}
		...
		transition_beforeLeave(el: HTMLElement) {
			...
			this.app_transition_out = this.$os.apps.find((x: AppInfo) => x.vendor + "_" + x.ident == el.dataset.appname);
			...
			switch(this.app_transition_type_out){
				...
				case Transition.SLIDE: {
					this.transition_out_class_pre = "transition-out-slide-pre";
					this.transition_out_class = "transition-out-slide";
					this.transition_out_class_in = "is-zoomed-out";
					this.transition_out_duraction = 1000;
					break;
				}
