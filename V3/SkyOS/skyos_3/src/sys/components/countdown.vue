...
export default Vue.extend({
	name: "countdown",
	props: ['time', 'short', 'full', 'capitalize', 'only_hours', 'no_fix', 'has_warn', 'warn_for', 'level_1', 'level_2'],
	data() {
		return {
			timeStr: "",
			diffM: 0,
			...
			if(!this.only_hours) {
				...
				if(this.diffM > 120) {
					if(this.short){
						...
						this.timeStr = pre + ' ' + this.diffH + ' ' + hoursStr + (this.diffH != 1 ? 's' : '') + ' ' + pro;
					}
				} else {
					if(this.short){
						this.timeStr = pre + ' ' + this.diffM + 'm ' + pro;
					} else {
						this.timeStr = pre + ' ' + this.diffM + ' ' + minutesStr + (this.diffM != 1 ? 's' : '') + ' ' + pro;
					}
				}
			}
			if(this.capitalize) {
