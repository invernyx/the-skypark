<template>
	<div class="collapser" :class="{ 'collapser_expanded': expanded, 'collapser_collapsed': !expanded, 'collapser_transition': doneTimeout != null }" v-if="!isReversed">
	...
	<div class="collapser collapser_reversed" :class="{ 'collapser_expanded': expanded, 'collapser_collapsed': !expanded, 'collapser_transition': doneTimeout != null }" v-else>
		<div class="collapser_content" ref="content">
			<div>
				<slot name="content" v-if="expanded || doneTimeout != null || animate || preload" :expanded="expanded"></slot>
			</div>
...
.collapser {
	...
	.collapser_content {
		...
		align-self: stretch;
		pointer-events: none;
		transform: translateY(-50px); //scale(0.98,0.98);
	...
	&_expanded {
		& > .collapser_content {
			...
			transition: transform 0.2s 0.1s cubic-bezier(0,.3,.2,1), height 0.3s cubic-bezier(.3,0,.2,1), opacity 0.2s 0.1s cubic-bezier(.3,0,.24,1); // in
		}
		& > .collapser_arrowed {
			&.collapser_title /deep/ .collapser_arrow {
