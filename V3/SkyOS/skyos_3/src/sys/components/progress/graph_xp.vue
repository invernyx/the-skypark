	...
	<div class="progress" v-if="progress" :data-thook="'progress_section_xp'">
		<div class="progress-box">
			<div class="progress-box--header">
				<div>
					...
					<div class="progress-box--balance">{{ rank.current.level != null ? rank.current.level : '?' }}</div>
				</div>
				<div>
					<div class="progress-box--title">XP</div>
					<div class="progress-box--balance">{{ progress.balance != undefined ? Math.round(progress.balance).toLocaleString('en-gb') : '?' }}</div>
				</div>
			</div>
		...
		<div class="progress-box">
			<div class="progress-box--steps">
				...
				<div class="progress-box--steps-level">{{ rank.current.level != null ? (rank.current.level + 1) : '?' }}</div>
				<div class="progress-box--steps-label-top">Level Up</div>
				<div class="progress-box--steps-label-bottom"><span>{{ rank.current.delta != null ? Math.round(rank.current.delta).toLocaleString('en-gb') : '?' }}</span>/<span>{{ rank.current.range != null ? rank.current.range.toLocaleString('en-gb') : '?' }}</span></div>
			</div>
		</div>
	</div>
</template>
	...
	methods: {
		...
		getRank() {
			if(this.progress.balance != null) {
				...
				this.progressInRank = this.rank.current.progress * 100;
				this.gainInRank = this.rank.current.gain_progress * 100;
	...
	.progress {
		...
		&.theme--bright {
			&-box {
				...
				&--header {
					text-shadow: 0 0 10px $ui_colors_bright_shade_0;
				}
				&--trend {
					svg {
		...
		&.theme--dark {
			&-box {
				...
				&--header {
					text-shadow: 0 0 10px $ui_colors_dark_shade_0;
				}
				&--trend {
					svg {
